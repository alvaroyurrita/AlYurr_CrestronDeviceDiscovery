...
{
    /// <summary> Gets all the IpV4 Adapters that are active in the computer </summary>
    ...
    public static List<IpV4NetworkAdapter> GetIpV4Adapters()
    {
        ...
        var operationalAdapters = ipV4Adapters.FindAll(
            a => a.OperationalStatus == OperationalStatus.Up && a.NetworkInterfaceType != NetworkInterfaceType.Loopback
        );
        var ipv4NetworkAdapters = operationalAdapters.Select(
                a =>
                {
                    var aUnicastAddress = a.GetIPProperties()
                        .UnicastAddresses.First(ad => ad.Address.AddressFamily == AddressFamily.InterNetwork);
